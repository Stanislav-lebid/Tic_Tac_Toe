...
{
	printf("\t\t\t Tic-Tac-Toe\n\n");
	printf("Виберіть клітинку, пронумеровану від 1 до 9, як показано нижче, і грайте\n\n");
...
void playTicTacToe(int whoseTurn)
{
	...
	while (!gameOver(board) && moveIndex != SIDE * SIDE) {
		if (whoseTurn == COMPUTER) {
			...
			printf("Комп'ютер поставив %c в клітинку %d %d\n",
			...
			int move;
			printf("Введіть клітинку свого ходу (1-9): ");
			scanf("%d", &move);
			if (move < 1 || move > 9) {
				printf("Неправильне введення! Будь ласка введіть "
					"число між 1 і 9.\n");
				continue;
			}
			x = (move - 1) / SIDE;
			...
			if (board[x][y] == ' ') {
				...
				printf("Клітинка %d занята!. Спробуйте"
					"знову.\n",
					move);
			}
		}
